...
NSString * k1504_AboutMe_Ogunjobi_Brandee$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1504_AboutMe_Ogunjobi_BrandeeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1504_aboutme_ogunjobi_brandee);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1504_aboutme_ogunjobi_brandee gc];
}
...
{
	// create k1504_AboutMe_Ogunjobi_Brandee global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1504_AboutMe_Ogunjobi_Brandee" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1504_aboutme_ogunjobi_brandee = [[k1504_AboutMe_Ogunjobi_BrandeeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1504_aboutme_ogunjobi_brandee];
	
	NSString *_k1504_aboutme_ogunjobi_brandeeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1504_aboutme_ogunjobi_brandeeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1504_aboutme_ogunjobi_brandee valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1504_AboutMe_Ogunjobi_Brandee$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1504_aboutme_ogunjobi_brandee.kroll" 
