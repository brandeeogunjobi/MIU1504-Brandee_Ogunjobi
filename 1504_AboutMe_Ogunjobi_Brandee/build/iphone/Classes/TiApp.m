...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1504_AboutMe_Ogunjobi_BrandeeUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1504_AboutMe_Ogunjobi_BrandeeUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1504_AboutMe_Ogunjobi_Brandee app using the node.js CLI
